 ...
 */
public class MediocreBoardEvaluation implements Definitions
{
	...
	*/
	...
	public static final int evaluate(MediocreBoard board) {
		...
		for(int i = 0; i < board.w_pawns.count; i++) {
			...
			rank = MediocreBoard.rank(index);
			file = MediocreBoard.file(index);
		...
		for(int i = 0; i < board.w_knights.count; i++) {
			...
			evalDetail.tropism.wm += TROPISM_KNIGHT[MediocreBoard.distance(board.b_king.pieces[0], index)];
		...
		for(int i = 0; i < board.b_knights.count; i++) {
			...
			evalDetail.tropism.bm += TROPISM_KNIGHT[MediocreBoard.distance(board.w_king.pieces[0], index)];
		...
		for(int i = 0; i < board.w_bishops.count; i++) {
			...
			evalDetail.tropism.wm += TROPISM_BISHOP[MediocreBoard.distance(board.b_king.pieces[0], index)];
			evalDetail.tropism.we = evalDetail.tropism.wm;
		}
		for(int i = 0; i < board.b_bishops.count; i++) {
			...
			evalDetail.tropism.bm += TROPISM_BISHOP[MediocreBoard.distance(board.w_king.pieces[0], index)];
			evalDetail.tropism.be = evalDetail.tropism.bm;
		}
		// Bishop pair bonus
		...
		for(int i = 0; i < board.w_rooks.count; i++) {
			...
			if(rank == 6 && (bPawnOnSeventh || MediocreBoard.rank(board.b_king.pieces[0]) == 7)) {
			...
			evalDetail.tropism.wm += TROPISM_ROOK[MediocreBoard.distance(board.b_king.pieces[0], index)];
			evalDetail.tropism.we = evalDetail.tropism.wm;
		...
		for(int i = 0; i < board.b_rooks.count; i++) {
			...
			if(rank == 1 && (wPawnOnSecond || MediocreBoard.rank(board.w_king.pieces[0]) == 0)) {
			...
			evalDetail.tropism.bm += TROPISM_ROOK[MediocreBoard.distance(board.w_king.pieces[0], index)];
			evalDetail.tropism.be = evalDetail.tropism.bm;
		}
		...
		for(int i = 0; i < board.w_queens.count; i++) {
			...
			// Queen on 7th
			if(MediocreBoard.rank(index) == 6 && (bPawnOnSeventh || MediocreBoard.rank(board.b_king.pieces[0]) == 7)) {
				evalDetail.piecePos.wm += QUEEN_ON_SEVENTH;
				evalDetail.piecePos.we += QUEEN_ON_SEVENTH;
			}
			
			evalDetail.tropism.wm += TROPISM_QUEEN[MediocreBoard.distance(board.b_king.pieces[0], index)];
			evalDetail.tropism.we = evalDetail.tropism.wm;
		}
		for(int i = 0; i < board.b_queens.count; i++) {
			...
			if(MediocreBoard.rank(index) == 1 && (wPawnOnSecond || MediocreBoard.rank(board.w_king.pieces[0]) == 0)) {
				evalDetail.piecePos.bm += QUEEN_ON_SEVENTH;
				evalDetail.piecePos.be += QUEEN_ON_SEVENTH;
			}

			evalDetail.tropism.bm += TROPISM_QUEEN[MediocreBoard.distance(board.w_king.pieces[0], index)];
			evalDetail.tropism.be = evalDetail.tropism.bm;
		}
		// Kings
	 ...
	 */
	public static final boolean drawByMaterial(MediocreBoard board, int side) {
		if(side == WHITE)
		{
	...
	public static final int drawProbabilityMiddle(MediocreBoard board, int totalEval) {
	 ...
	 */
	public static final int drawProbabilityEnding(MediocreBoard board, int totalEval) {
		int adjustedScore = totalEval;
		...
		{
			w_rank = MediocreBoard.rank(board.w_bishops.pieces[0]);
			w_row = MediocreBoard.file(board.w_bishops.pieces[0]);
			b_rank = MediocreBoard.rank(board.b_bishops.pieces[0]);
			b_row = MediocreBoard.file(board.b_bishops.pieces[0]);
	 ...
	 */
	public static final void w_kingDefense(MediocreBoard board) {
		int totalDefense = 0;
		int pawnRank;
		int king_index = board.w_king.pieces[0];
		int king_file = MediocreBoard.file(king_index);
		int oking_file = MediocreBoard.file(board.b_king.pieces[0]); // Opponent's king
	 ...
	 * @return totalDefense The total defense value of the king
	 */
	public static final void b_kingDefense(MediocreBoard board) {
		int totalDefense = 0;
		int pawnRank;
		int king_index = board.b_king.pieces[0];
		int king_file = MediocreBoard.file(king_index);
		int oking_file = MediocreBoard.file(board.w_king.pieces[0]); // Opponent's king
	 ...
	 */
	public static final void w_kingAttacked(MediocreBoard board) {
		int totalAttack = 0;
		int attackedCount = 0;
		byte flag = 0;
	 ...
	 * @return totalAttack The total value from the attacking pieces (is negated before returned so it can be added to kingSafety)
	 */
	public static final void b_kingAttacked(MediocreBoard board) {
		int totalAttack = 0;
		int attackedCount = 0;
		byte flag = 0;
	...
	public static final Value evaluatePassers(MediocreBoard board) {
		...
		while(whitePassers != 0)
		{
			...
			whiteEvalMiddle += (rankBonusMiddle * Math.abs(MediocreBoard.file(board.b_king.pieces[0]) - file))/14;
			whiteEvalEnding += (rankBonusEnding * Math.abs(MediocreBoard.file(board.b_king.pieces[0]) - file))/10;
			...
			if(gamePhase == PHASE_PAWN_ENDING)
			{
				...
				if(MediocreBoard.file(board.w_king.pieces[0]) == file && MediocreBoard.rank(board.w_king.pieces[0]) > rank) promDist++;
				...
				if(promDist < Math.max(Math.abs(MediocreBoard.rank(board.b_king.pieces[0])-rank), Math.abs(MediocreBoard.file(board.b_king.pieces[0])-file)))
				{
		...
		while(blackPassers != 0)
		{
			...
			blackEvalMiddle += (rankBonusMiddle * Math.abs(MediocreBoard.file(board.w_king.pieces[0]) - file))/14;
			blackEvalEnding += (rankBonusEnding * Math.abs(MediocreBoard.file(board.w_king.pieces[0]) - file))/10;

			if(gamePhase == PHASE_PAWN_ENDING)
			{
				promDist = rank; 
				if(board.toMove == WHITE_TO_MOVE) promDist++;
				if(MediocreBoard.file(board.b_king.pieces[0]) == file && MediocreBoard.rank(board.b_king.pieces[0]) < rank) promDist++;
				if(rank == 6) promDist--;
				if(promDist < Math.max(Math.abs(MediocreBoard.rank(board.w_king.pieces[0])-rank), Math.abs(MediocreBoard.file(board.w_king.pieces[0])-file)))
				{
	 ...
	 */
	public static final Value pawnEval(MediocreBoard board) {
	 ...
	 */
	public static final void gen_attack_wknight(MediocreBoard board, int square) {
		int mobility_all = 0;
		int mobility_safe = 0;
	 ...
	 * @return mobility_total The total mobility value of the piece
	 */
	public static final void gen_attack_bknight(MediocreBoard board, int square) {
		int mobility_all = 0;
		int mobility_safe = 0;
		int mobility_total = 0;
		int attackedSquare;
		...
		if(mobility_safe == 1)
		{
			// A 'trapped' piece further up on the board is worse than closer to home
			// since it risks being captured further up
			mobility_total -= ((7- MediocreBoard.rank(square)+1) *5)/2;
		}
		// If the piece have no safe squares it is just as good as trapped so penalize
		// this even harder
		else if(mobility_safe == 0)
		{
			mobility_total -= ((7- MediocreBoard.rank(square)+1)*5);
		}
	 ...
	 */
	public static final void gen_attack_wbishop(MediocreBoard board, int square) {
		int mobility_all = 0;
		int mobility_safe = 0;
		int mobility_total = 0;
		int attackedSquare;
	 ...
	 * @return mobility_total The total mobility score of the piece
	 */
	public static final void gen_attack_bbishop(MediocreBoard board, int square) {
		int mobility_all = 0;
		int mobility_safe = 0;
		int mobility_total = 0;
		int attackedSquare;
	 ...
	 * @return mobility_total The total mobility score of the piece
	 */
	public static final void gen_attack_wrook(MediocreBoard board, int square) {
		int mobility_all = 0;
		int mobility_safe = 0;
		int mobility_total = 0;
		int attackedSquare;
	 ...
	 * @return mobility_total The total mobility score of the piece
	 */
	public static final void gen_attack_brook(MediocreBoard board, int square) {
		int mobility_all = 0;
		int mobility_safe = 0;
		int mobility_total = 0;
		int attackedSquare;
	 ...
	 * @return mobility_total The total mobility score of the piece
	 */
	public static final void gen_attack_wqueen(MediocreBoard board, int square) {
		int mobility_all = 0;
		int mobility_safe = 0;
		int mobility_total = 0;
		int attackedSquare;
	 ...
	 * @return mobility_total The total mobility score of the piece
	 */
	public static final void gen_attack_bqueen(MediocreBoard board, int square) {
		int mobility_all = 0;
		int mobility_safe = 0;
		int mobility_total = 0;
		int attackedSquare;
	 ...
	 * @return mobility_total The total mobility value of the piece
	 */
	public static final void gen_attack_wking(MediocreBoard board, int square) {
		int attackedSquare;
	 ...
	 * @return mobility_total The total mobility value of the piece
	 */
	public static final void gen_attack_bking(MediocreBoard board, int square) {
		int attackedSquare;
	 ...
	 */
	public static final int w_trapped(MediocreBoard board) {
		int trapped_eval = 0;
	 ...
	 * @return trapped_eval The penalty if one or more patterns exists 0 if no exist
	 */
	public static final int b_trapped(MediocreBoard board) {
		int trapped_eval = 0;
	 ...
	 */	
	public static final int getGamePhase(MediocreBoard inputBoard) {
	...
	public static class EvalDetail {
		...
		public String whiteMiddleStats() {
			int total = totalWhiteMiddleEval();
			int mat = material.wm;
			int mob = mobility.wm;
			int pos = piecePos.wm;
			int tro = tropism.wm;
			int tra = trappedEval.wm;
			int paw = pawnStructure.wm;
			int pas = passerEval.wm;
			int kat = kingAttacked.wm;
			int kde = kingDefense.wm;
			int tem = tempoEval.wm;
			return String.format("%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d", total, mat, mob, pos, tro, tra, paw, pas, kat, kde, tem);
		}

		public String whiteEndStats() {
			int total = totalWhiteEndingEval();
			int mat = material.we;
			int mob = mobility.we;
			int pos = piecePos.we;
			int tro = tropism.we;
			int tra = trappedEval.we;
			int paw = pawnStructure.we;
			int pas = passerEval.we;
			int kat = kingAttacked.we;
			int kde = kingDefense.we;
			int tem = tempoEval.we;
			return String.format("%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d", total, mat, mob, pos, tro, tra, paw, pas, kat, kde, tem);
		}

		public String blackMiddleStats() {
			int total = totalBlackMiddleEval();
			int mat = material.bm;
			int mob = mobility.bm;
			int pos = piecePos.bm;
			int tro = tropism.bm;
			int tra = trappedEval.bm;
			int paw = pawnStructure.bm;
			int pas = passerEval.bm;
			int kat = kingAttacked.bm;
			int kde = kingDefense.bm;
			int tem = tempoEval.bm;
			return String.format("%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d", total, mat, mob, pos, tro, tra, paw, pas, kat, kde, tem);
		}

		public String blackEndStats() {
			int total = totalBlackEndingEval();
			int mat = material.be;
			int mob = mobility.be;
			int pos = piecePos.be;
			int tro = tropism.be;
			int tra = trappedEval.be;
			int paw = pawnStructure.be;
			int pas = passerEval.be;
			int kat = kingAttacked.be;
			int kde = kingDefense.be;
			int tem = tempoEval.be;
			return String.format("%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d", total, mat, mob, pos, tro, tra, paw, pas, kat, kde, tem);
		}
