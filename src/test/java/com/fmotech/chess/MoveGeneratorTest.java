...
import com.fmotech.chess.oli.OliThink;
...
public class MoveGeneratorTest {
    ...
    public void shouldDetectPawnLastRowAttack() {
        ...
        assertHasMove(moveFromFen(board, "d7c8q"), moves);
        Board kill = board.move(moveFromFen(board, "d7c8q"));
        assertFalse(MoveGenerator.positionAttacked(lowestBitPosition(board.ownKing()), board));
        assertFalse(MoveGenerator.positionAttacked(lowestBitPosition(kill.ownKing()), kill));
    }

    public static void assertHasMove(int move, int[] moves) {
    ...
    public void debugPerft() {
        String fen = "K7/8/1Q6/8/8/8/7k/7q w - - 5 4";
//        String fen = "r3k2r/p1ppqpb1/bn2pnp1/3PN3/1p2P3/2N2Q1p/PPPBBPPP/R3K1R1 b Qkq - 1 1";
        int depth = 2;

        long l = MoveGenerator.countMoves(depth, Board.fen(fen), true);
